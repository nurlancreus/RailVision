...
namespace RailVision.Application.Abstractions
{
    ...
    {
        Task<IEnumerable<PopulationCenterDTO>> GetAllAsync(string? searchQuery, int? minPopulation, int? maxPopulation, bool? isDistinct = true, CancellationToken cancellationToken = default);
